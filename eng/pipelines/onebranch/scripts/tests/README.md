...
| XML docs unexpected elements | Member containers emitted by an over-broad `<include>` reported; compiler-supported top-level documentation elements accepted |
| XML docs pipeline invocation | Source validation enabled by each job's configured snippet path; SqlClient, SqlServer, Abstractions, and Azure snippet directories wired and present |
