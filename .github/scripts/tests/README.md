...
| `cherry-pick-to-release.bats` | 19 | Branch derivation, already-applied detection, clean cherry-pick, conflict handling, milestone lookup, PR creation, duplicate skip logic, backport-issue lookup and `Fixes #` linking |
...
| `create-backport-issue.bats` | 10 | Hotfix label validation, duplicate backport-issue detection (by milestone or title prefix), milestone lookup, child issue creation, and native sub-issue linking |
| `sync-hotfix-label-to-pr.bats` | 7 | Reading a PR's closing issue references, collecting `Hotfix X.Y.Z` labels from those issues, idempotent label sync onto the PR |
