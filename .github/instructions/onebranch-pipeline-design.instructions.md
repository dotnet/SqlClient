...
- Gate categories are derived from `isOfficial`: `error`, `missing-symbols`, and `dependency-inconsistency` always, plus `delay-signed`, `unsigned`, and `package-unsigned` on official runs only. The `error` severity covers only error-severity findings, so each warning/info category must be named explicitly — `missing-symbols`, `dependency-inconsistency`, and `delay-signed` are warnings, and `unsigned` and `package-unsigned` are info. Non-official runs are deliberately unsigned and delay-signed, so gating the three signing categories there would always fail
