...
When asked to investigate a bug or issue in Microsoft.Data.SqlClient, follow this workflow:
...
- **Cover both sync and async code paths** if the affected API has both variants (e.g., `Open`/`OpenAsync`, `ExecuteReader`/`ExecuteReaderAsync`).
