...
## Preview Release 7.0.0-preview2[@TODO:buildnumber] - 2025-10-15
...
- Applications connection to SQL Server BAGs using TCP and custom ports, especially where the server's provided partner name lacks the protocol, host, or port. This avoids connection failures when the server-provided partner is incompatible or incomplete.
...
- Then, ensure your connection string includes you preferred failover partner (with correct `tcp:host,port`) so that the client uses that instead of the server's suggestion.
