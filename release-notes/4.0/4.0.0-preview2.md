...
- Added support for **localdb** `shared instance` using managed SNI. [#1237](https://github.com/dotnet/SqlClient/pull/1237) [Read more](#sqllocaldb-support-is-added-to-managed-sni)
...
### SqlLocalDb support is added to Managed SNI

This feature enables users to setup a minimal set of files necessary to start the SQL Server Database Engine. Data Source for **localdb** should always start with `(localdb)`.

- Possible scenarios:

  - `(localdb)\.` This will stablish a connection to default (automatic) instance of localdb which is MSSQLLocalDb
