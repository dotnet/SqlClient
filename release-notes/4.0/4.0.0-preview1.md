...
When set to true parameter names will not be sent to the SQL server when the command is executed. This behavior has been shown to increase performance for commands with very large numbers of parameters which are used in bulk operations. This also means that you can't reference parameters by name when using this option.
