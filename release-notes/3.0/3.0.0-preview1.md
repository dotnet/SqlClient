...
## Microsoft.Data.SqlClient 3.0.0-preview1.xxxxx.x released 15 March 2021
...
This new feature introduces support to allow client applications to retry on "transient" or "retriable" errors, through code or app config files, when opening a connection or executing a command. This feature is disabled by default and is currently in preview. To enable this support, client applications must turn on the below safety switch:

`AppContext.SetSwitch("Switch.Microsoft.Data.SqlClient.EnableRetryLogic", true);`

Once .NET AppContext safety switch is on, a retry logic policy can be defined for `SqlConnection` and `SqlCommand` independently, or together various customization options.

New public APIs introduced to `SqlConnection` and `SqlCommand` for registering `SqlRetryLogicBaseProvider` implementation:
...
New configuration sections have also been introduced to do the same registration from configuration files alternatively, without modifying existing code:
...
A simple example to use available configuration sections in configuration files is below:
...
Alternatively applications can also implement their own provider over the `SqlRetryLogicBaseProvider` base class, and register it with `SqlConnection`/`SqlCommand`.
...
These counters can be used with .NET Core global CLI tools: `dotnet-counters` and `dotnet-trace` in Windows or Linux and PerfView in Windows, with `Microsoft.Data.SqlClient.EventSource` as process name.
