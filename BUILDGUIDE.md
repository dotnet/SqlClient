...
|AZURE_KEY_VAULT_KEY_URL | (Optional) Azure Key Vault Key Identifier URL | `https://{keyvaultname}.vault.azure.net/keys/{KeyName}/{KeyId}` |
|AZURE_KEY_VAULT_CLIENT_ID | (Optional) "Application (client) ID" of Active Directory registered application, granted access to Azure Key Vault specified in `AZURE_KEY_VAULT_KEY_URL` | _{Client Application ID}_ |
|AZURE_KEY_VAULT_CLIENT_KEY | (Optional) "Client Secret" of Active Directory registered application, granted access to Azure Key Vault specified in `AZURE_KEY_VAULT_KEY_URL` | _{Client Application Secret}_ |
