...
3. **Explicit provider registration** — `SqlAuthenticationProviderManager.SetProvider()`
...
```bash
dotnet publish -c Release -p:EnableReflectionBasedAuthProviderDiscovery=true
```
...
SqlAuthenticationProviderManager API checks:
