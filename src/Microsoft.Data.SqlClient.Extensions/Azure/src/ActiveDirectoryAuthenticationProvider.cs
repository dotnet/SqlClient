    ...
    public override async Task<SqlAuthenticationTokenBase> AcquireTokenAsync(SqlAuthenticationParametersBase parameters)
    {
        ...
        **/
        ...
        if (parameters.AuthenticationMethod == SqlAuthenticationMethod.ActiveDirectoryDefault)
        {
            ...
            return new Token(accessToken.Token, accessToken.ExpiresOn);
        }
...
#pragma warning restore CS0618 // Type or member is obsolete
        ...
        if (result is null)
        {
            throw SQL.ActiveDirectoryFailedToAcquireToken(parameters.AuthenticationMethod.ToString());
        }

        return new Token(result.AccessToken, result.ExpiresOn);
    }
...
{
    public void LogInfo(string a, string b, string c)
    {
    }
}
...
internal class Token : SqlAuthenticationTokenBase
{
    public Token(string token, DateTimeOffset expiresOn) : base(token, expiresOn)
    {
    }
}
...
{
    internal static Exception UnsupportedAuthenticationSpecified(SqlAuthenticationMethod method)
        => new NotSupportedException($"The authentication method '{method}' is not supported.");

    internal static Exception ActiveDirectoryFailedToAcquireToken(string method)
        => new InvalidOperationException($"Failed to acquire access token for authentication method '{method}'.");

    internal static Exception ActiveDirectoryInteractiveTimeout()
        => new TimeoutException("Timed out while acquiring access token for Active Directory Interactive authentication.");

    internal static Exception ActiveDirectoryDeviceFlowTimeout()
        => new TimeoutException("Timed out while acquiring access token for Active Directory Device Code Flow authentication.");
}
