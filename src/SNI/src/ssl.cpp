...
DWORD CryptoBase::Close(DWORD dwFlags)
{
	BidxScopeAutoSNI1( SNIAPI_TAG _T("%u#\n"), GetBidId() );
...
DWORD CryptoBase::ReadAsync(__deref_inout SNI_Packet ** ppNewPacket, __in LPVOID pPacketKey)
{
	...
	if( SSL_REMOVED == m_State )
	{
		...
		// UNDONE: see SQLBU 398608 for more details. The use of 
		// InterlockedCompareExchangePointer() here is incorrect. If one thread
		// reads m_pLeftOver (3rd arg) and another thread then changes m_pLeftOver
		// to a new, non-NULL value, then the operation will fail. pTmpPacket
		// will get the value of m_pLeftOver, and m_pLeftOver will remain unchanged.
		// This would result in freeing the left over packet below, but a pointer
		// would remain in m_pLeftOver to be processed again later.
...
Ssl::~Ssl()
{
	...
	if (m_wchKeyContainer[0] != 0)
	{
		HCRYPTPROV hDummyProv;
		...
		CryptAcquireContextW(
			...
			PROV_RSA_SCHANNEL,
			CRYPT_DELETEKEYSET | m_dwKeyContainerFlags);
	}
}
										...
										__out SNIAuthProviderInfo *pAuthInfo)
{
	...
	if (pAuthInfo != NULL && pAuthInfo->pwszCertId != NULL)
	{
		dwRet = AcquireCertContext(pAuthInfo, pdwKeyContainerFlags, cchKeyContainer, pwchKeyContainer, phCertStore, ppCertContext);
		if (ERROR_SUCCESS == dwRet)
		{
			SchannelCred.cCreds = 1;
			SchannelCred.paCred = ppCertContext;
		}
	}
...
DWORD Ssl::AcquireCertContext(__in SNIAuthProviderInfo *pAuthInfo,
	__out DWORD *pdwKeyContainerFlags,
	__in ULONG cchKeyContainer,
	__out_ecount(cchKeyContainer) WCHAR *pwchKeyContainer,
	__out HCERTSTORE *phCertStore,
	__deref_out PCCERT_CONTEXT * ppCertContext)
{
	...
	if (pAuthInfo->fCertHash)
	{
		DWORD cbHash = sizeof(szHash);
		if (!CryptStringToBinaryW(pAuthInfo->pwszCertId, 0, CRYPT_STRING_HEX, szHash, &cbHash, NULL, NULL))
		{
			dwRet = GetLastError();
			return dwRet;
		}

		cryptHashBlob.cbData = sizeof(szHash);
		cryptHashBlob.pbData = szHash;
		pvFindPara = &cryptHashBlob;
		dwFindType = CERT_FIND_SHA1_HASH;
	}
	else
	{
		pvFindPara = (PVOID)pAuthInfo->pwszCertId;
	...
	// Using the "MY" store...
	//
	if (*phCertStore = CertOpenStore(CERT_STORE_PROV_SYSTEM,
		X509_ASN_ENCODING,
		0,
		CERT_SYSTEM_STORE_LOCAL_MACHINE | CERT_STORE_READONLY_FLAG,
		L"MY"))
	{
		dwRet = FindAndLoadCertificate(*phCertStore, dwFindType, pvFindPara, ppCertContext, FALSE);
		if (dwRet != ERROR_SUCCESS)
		{
	...
	if (dwRet != ERROR_SUCCESS)
	{
		...
		//
		if (*phCertStore = CertOpenSystemStoreA(0, "MY"))
		{
			dwRet = FindAndLoadCertificate(*phCertStore, dwFindType, pvFindPara, ppCertContext, FALSE);
	...
	if (dwRet != ERROR_SUCCESS)
	{
		if (pAuthInfo->pfnCertificateCallback != NULL)
		{
			dwRet = (*pAuthInfo->pfnCertificateCallback)(pAuthInfo->pCallbackContext,
				pAuthInfo->fCertHash,
				pAuthInfo->pwszCertId,
				ppCertContext,
				pdwKeyContainerFlags,
				cchKeyContainer,
				pwchKeyContainer);
		}
	}
										 ...
										 )
{
	BidxScopeAutoSNI4( SNIAPI_TAG _T("hMyCertStore: %p{HCERTSTORE}, ")
							  _T("dwFindType: %d{DWORD}, ")
							  _T("pvFindPara: %p, ")
							  _T("ppCertContext: %p{PCCERT_CONTEXT*}\n"), 
							  hMyCertStore, dwFindType, pvFindPara, ppCertContext);
	...
	while(1)
	{
		...
		if (dwFindType == CERT_FIND_SHA1_HASH || ERROR_SUCCESS == IsValidAuthCert(pCertContext, fServer))
		{
			if (fServer)
			{
...
DWORD Ssl::Initialize( PSNI_PROVIDER_INFO pInfo )
{
	...
	dwRet = AcquireCredentialsForClient(&s_hClientCred, NULL, 0, NULL, NULL, NULL, NULL);
	
	if( SEC_E_OK != dwRet )
		goto ErrorExit;


	dwRet = AcquireCredentialsForClient(&s_hClientCredValidate, NULL, 0, NULL, NULL, NULL, NULL);
...
// since this is a server side call
	...
	if( hCertStore = CertOpenStore(	CERT_STORE_PROV_SYSTEM,
									...
									L"MY"))
	{
		dwRet = FindAndLoadCertificate(hCertStore, dwFindType, pvFindPara, &s_hCertContext, TRUE);
...
DWORD Ssl::InitX(__in LPVOID pUserOption)
{
	...
	SNIAuthProviderInfo* pAuthInfo = (SNIAuthProviderInfo*)pUserOption;
	...
	DWORD dwOption = pAuthInfo->dwFlags;
	...
	if( m_pConn->m_fClient && !m_fUseExistingCred )
	{
		dwRet = AcquireCredentialsForClient(&m_hCred,
			&m_dwKeyContainerFlags,
			sizeof(m_wchKeyContainer) / sizeof(m_wchKeyContainer[0]),
			m_wchKeyContainer,
			&m_hCertStore,
			&m_pCertContext,
			pAuthInfo);
...
DWORD Ssl::IsValidAuthCert(PCCERT_CONTEXT   pCertContext, BOOL fServer)
{
...
Ssl::Ssl(SNI_Conn * pConn):CryptoBase( pConn )
{
	...
	m_hCertStore = NULL;
	m_pCertContext = NULL;
	m_wchKeyContainer[0] = 0;
	m_dwKeyContainerFlags = 0;
