...
#ifdef SNI_BASED_CLIENT		
	// Per blained, an AppDomain unload is interrupted
				   ...
				   int				   timeout)
{
	...
	// TODO: Fake a sync Open for now - change this later
	switch( ProvNum )
	{
...
DWORD SNIClose(__inout SNI_Conn * pConn)
{
...

	{
		...
		dwError = pConn->m_pProvHead->Close(0);
	}
...
//
// PURPOSE:
//    Similar to SNIGetInfo, Query SNIConn info, with buffer boundary checking
//
// RETURNS: error code
//
// NOTES:
//
DWORD SNIGetInfoEx (SNI_Conn * pConn, UINT QType, __out_bcount(cbBuf)PVOID pbQInfo, USHORT cbBuf)
{
	BidxScopeAutoSNI4( SNIAPI_TAG _T( "pConn: %p{SNI_Conn*}, QType: %d, pbQInfo: %p, cbBuf: %d\n"), 
				pConn, QType, pbQInfo, cbBuf);
	...
	switch(QType)
	{
		case SNI_QUERY_CONN_IPADDR:
			{
				CAutoSNICritSec a_cs(pConn->m_csProvList, SNI_AUTOCS_ENTER);

				// Loop through providers until we find tcp provider
				//
				for( pProv = pConn->m_pProvHead; pProv; pProv = pProv->m_pNext)
				{
					...
					if( pProv->m_Prot == TCP_PROV )
					{
						...
						DWORD dwSockErr = ((Tcp *)pProv)->GetPeerIPAddr((PSOCKADDR)pbQInfo, &cbAddr);
						if(dwSockErr != 0)
						{
...
DWORD SNIGetInfo( __in SNI_Conn * pConn, UINT QType, __out VOID * pbQInfo)
{
	...
	switch(QType)
	{
		...
		case SNI_QUERY_CONN_SSL_SECCTXTHANDLE:

			{
				PCtxtHandle **pph = (PCtxtHandle **)(pbQInfo);
				*pph = NULL;
				dwError = ERROR_INVALID_PARAMETER;
				...
				for( pProv = pConn->m_pProvHead; pProv; pProv = pProv->m_pNext)
				{
					if( (pProv->m_Prot == SSL_PROV) && pProv->m_fActive )
					{
						dwError = pProv->GetContextHandle((PVOID*)pph);
						break;
					}
				}
			}
...
DWORD SNIAddProvider(__inout SNI_Conn * pConn, ProviderNum ProvNum, __in LPVOID pInfo)
{
	...
	switch( ProvNum )
	{
        ...
        case CTAIP_PROV:
            ...
            if(NULL == (pProv = NewNoX(gpmo) CrTrAdditionalInfoProtocol(pConn)))
            {
                SNI_SET_LAST_ERROR( INVALID_PROV, SNIE_4, ERROR_OUTOFMEMORY );
	...
	switch( ProvNum )
	{
        ...
        case CTAIP_PROV:
            dwError = static_cast<CrTrAdditionalInfoProtocol *>(pProv)->InitX(pInfo);
            break;
...
#else
	//swallow this up only in retail bits
...
//also, dont swallow any exceptions until we decide on what to do.
int SNIExceptionFilter(LPEXCEPTION_POINTERS pExc)
{
    #ifdef DEBUG
        return EXCEPTION_CONTINUE_SEARCH;
