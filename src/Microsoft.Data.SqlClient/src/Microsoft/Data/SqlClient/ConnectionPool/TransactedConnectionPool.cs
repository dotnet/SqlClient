...
internal class TransactedConnectionPool
{
    ...
    {
        TransactedConnections = new ConcurrentDictionary<Transaction, List<DbConnectionInternal>>();
    }
    ...
    internal DbConnectionInternal? GetTransactedObject(Transaction transaction)
    {
        ...
        if (TransactedConnections.TryGetValue(
            transaction,
        ...
        if (!txnFound)
        {
            lock (TransactedConnections)
            {
                ...
                else
                {
                    ...
                    TransactedConnections.TryAdd(transaction, newConnections);
                }
            }
