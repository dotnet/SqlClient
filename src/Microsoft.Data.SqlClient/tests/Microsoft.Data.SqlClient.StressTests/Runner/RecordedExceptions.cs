...
namespace DPStressHarness
{
    public class RecordedExceptions
    {
        ...
        public void TraceAllExceptions()
        {
            ...
            status.AppendLine(string.Format("Total test(s) with exception: {0}", _exceptions.Count));
            status.AppendLine("========================================================================");
            ...
            foreach (string testName in _exceptions.Keys)
            {
                ...
                {
                    status.AppendLine(string.Format(" No: {0} of {1} [{2}]", count ++, exceptionsForTest.Count, testName));
