    ...
    public class StressEngine
    {
        ...
        public void RunStressThread()
        {
            try
            {
                ...
                long testDuration = _duration * Stopwatch.Frequency;
                ...
                while (_continue && timer.ElapsedTicks < testDuration)
                {
                    ...
                    StressTest test = tests[weightedLookup[n]];

                    if (TestMetrics.PrintMethodName)
                    {
                        FakeConsole.WriteLine("{0}: {1}", ++s_globalRequestsCounter, test.Title);
                    }
                    ...
                    catch (Exception e)
                    {
                        ...
                        bool thresholdExceeded = _exceptions.Record(test.Title, e);
                        if (thresholdExceeded)
                        {
                            FakeConsole.WriteLine("Exception Threshold of {0} has been exceeded on {1} - Halting!\n",
                                TestMetrics.ExceptionThreshold, test.Title);
                            break;
                        }
                    }
                    finally
