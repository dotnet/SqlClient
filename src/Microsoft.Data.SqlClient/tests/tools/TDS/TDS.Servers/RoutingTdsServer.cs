...
namespace Microsoft.SqlServer.TDS.Servers
{
    ...
    public class RoutingTdsServer : GenericTdsServer<RoutingTdsServerArguments>
    {
        ...
        public override TDSMessageCollection OnPreLoginRequest(ITDSServerSession session, TDSMessage request)
        {
            ...
            if (Arguments.RouteOnPacket == TDSMessageType.TDS7Login)
            {
        ...
        public override TDSMessageCollection OnLogin7Request(ITDSServerSession session, TDSMessage request)
        {
            ...
            // Check filter
            if (Arguments.RequireReadOnly && (loginRequest.TypeFlags.ReadOnlyIntent != TDSLogin7TypeFlagsReadOnlyIntent.ReadOnly))
            {
                // Log request
        ...
        public override TDSMessageCollection OnSQLBatchRequest(ITDSServerSession session, TDSMessage request)
        {
            ...
            if (Arguments.RouteOnPacket == TDSMessageType.SQLBatch)
            {
