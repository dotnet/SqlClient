  ...
  try {
    ...
    if ($OS -eq "Unix") {
        ...
        if ($fqdn.length -gt 64) {
            openssl req -x509 -newkey rsa:4096 -sha256 -days 1095 -nodes -keyout $OutDir/localhostcert.key -out $OutDir/localhostcert.cer -addext "subjectAltName=DNS:$fqdn,DNS:localhost,IP:127.0.0.1,IP:::1"
        }
        else {
