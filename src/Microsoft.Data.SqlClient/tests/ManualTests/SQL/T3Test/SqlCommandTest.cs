...
namespace Microsoft.Data.SqlClient.ManualTesting.Tests
{
    public static class T3SqlCommandTest
    {
        ...
        /**
         * This is a superficial test just to make sure TransactionScope works.
         * 
         * I personally don't use TransactionScope ever so this might have room for improvement.
         */
        [CheckConnStrSetupFact]
        public static void TransactionScopeTest()
        {
            const string table = "TransactionScopeTest";
            ...
            using (SqlConnection gcon = new SqlConnection(T3.ConnectionString))
            {
                gcon.Open();

                T3.DropTable(gcon, table);
                T3.CreateTable(gcon, table, false);

                try
                {
                    // test TransactionScope + rollback
                    Assert.Throws<InvalidProgramException>(new Action(delegate
                    {
                        using (var scope = new TransactionScope(TransactionScopeOption.RequiresNew, scopeOptions))
                        using (var lcon = new SqlConnection(T3.ConnectionString))
                        {
                            lcon.Open();
                            Assert.Equal(1, T3.InsertTable(lcon, table));
                            Assert.Equal(1, T3.CountTable(lcon, table));
                            throw new InvalidProgramException();
                        }
                    }));
                    Assert.Equal(0, T3.CountTable(gcon, table));

                    // test TransactionScope + commit
                    using (var scope = new TransactionScope(TransactionScopeOption.RequiresNew, scopeOptions))
                    using (var lcon = new SqlConnection(T3.ConnectionString))
                    {
                        lcon.Open();
                    ...
                    Assert.Equal(1, T3.CountTable(gcon, table));
                }
                finally
                {
                    T3.DropTable(gcon, table);
