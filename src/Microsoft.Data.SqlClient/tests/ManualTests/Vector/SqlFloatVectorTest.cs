...
{
    public class SqlFloatVectorTest
    {
        [Fact]
        public void Constructor_WithLength_InitializesCorrectly()
        {
            var vec = new SqlFloatVector(4);
            Assert.Equal(4, vec.ElementCount);
        ...
        {
            float[] data = { 1f, 2f, 3f };
            var vec = new SqlFloatVector(data);
            Assert.Equal(3, vec.ElementCount);
        ...
        {
            Assert.Throws<ArgumentException>(() => new SqlFloatVector(null!));
        }
        ...
        public void ToArray_ReturnsOriginalFloats()
        {
            ...
            var vec = new SqlFloatVector(input);
        ...
        {
            var vecNull = SqlFloatVector.Null;
            var vecReal = new SqlFloatVector(new float[] { 1f });
        ...
        {
            var vec = SqlFloatVector.Null;
            Assert.Equal(0, vec.ElementCount);
        }
        ...
        public void Internal_Constructor_WithValidRawBytes_DeserializesCorrectly()
        {
            ...
            var original = new SqlFloatVector(data);
            var raw = GetISqlVectorProperty<byte[]>(original, "VectorPayload");

            var ctor = typeof(SqlFloatVector).GetConstructor(
                BindingFlags.NonPublic | BindingFlags.Instance,
                null, new[] { typeof(byte[]) }, null);
            Assert.NotNull(ctor);

            var deserialized = (SqlFloatVector)ctor.Invoke(new object[] { raw });
        ...
        {
            var vec = new SqlFloatVector(new float[] { 1f, 2f });
        ...
        {
            var vec = new SqlFloatVector(new float[] { 1f });
        ...
        {
            var vec = new SqlFloatVector(new float[] { 10f, 20f });
        ...
        private static T GetISqlVectorProperty<T>(SqlFloatVector instance, string propName)
        {
            var interfaceType = typeof(SqlFloatVector).GetInterface("Microsoft.Data.SqlClient.ISqlVector", true);
