...
namespace Microsoft.Data.SqlClient
{
    ...
    internal sealed partial class TdsParser
    {
        ...
        internal TdsOperationStatus TryProcessReturnValue(int length, TdsParserStateObject stateObj, out SqlReturnValue returnValue, SqlCommandColumnEncryptionSetting columnEncryptionSetting)
        {
            ...
            else if (rec._metaType.IsCharType)
            {
        ...
        private TdsOperationStatus TryProcessRow(_SqlMetaDataSet columns, object[] buffer, int[] map, TdsParserStateObject stateObj)
        {
            ...
            for (int i = 0; i < columns.Length; i++)
            {
                ...
                else
                {
                    ...
                    result = TryReadSqlValue(data, md, md._metaType.IsPlp ? (Int32.MaxValue) : (int)len, stateObj, SqlCommandColumnEncryptionSetting.Disabled /*Column Encryption Disabled for Bulk Copy*/, md._column);
                    if (result != TdsOperationStatus.Done)
                    {
                        return result;
                    }
                                                ...
                                                bool write /* if false just calculates the length */)
        {
            ...
            switch (fedAuthFeatureData._libraryType)
            {
                ...
                case TdsEnums.FedAuthLibrary.SecurityToken:
                    ...
                    dataLen = 1 + sizeof(int) + fedAuthFeatureData._accessToken.Length; // length of feature data = 1 byte for library and echo, security token length and sizeof(int) for token length itself
                    break;
                default:
                                     ...
                                     uint outSSPILength)
        {
            try
            {
                ...
                */
                ...
                // 4th one
                if (!string.IsNullOrEmpty(rec._newPassword) || (rec._newSecurePassword != null && rec._newSecurePassword.Length != 0))
                {
                    log7Flags |= 1 << 24;
                }
                ...
                try
                {
                    ...
                    for (int ii = startRpc; ii < rpcArray.Count; ii++)
                    {
                        ...
                        if (startParam == 0 || ii > startRpc)
                        {
                            ...
                            {
                                Debug.Assert(!string.IsNullOrEmpty(rpcext._rpcName), "must have an RPC name");
        ...
        internal void WriteTceUserTypeAndTypeInfo(SqlMetaDataPriv mdPriv, TdsParserStateObject stateObj)
        {
            ...
            switch (mdPriv._type)
            {
                ...
                default:
                    WriteTokenLength(mdPriv._tdsType, mdPriv._length, stateObj);
                    if (mdPriv._metaType.IsCharType)
                    {
