...
namespace Microsoft.Data.SqlClient
{
    ...
    internal sealed partial class TdsParser
    {
        ...
        private PreLoginHandshakeStatus ConsumePreLoginHandshake(bool encrypt, bool trustServerCert, bool integratedSecurity, out bool marsCapable, out bool fedAuthRequired)
        {
            ...
            OperationStatus result = _physicalStateObj.TryReadNetworkPacket();
            ...
            if (_physicalStateObj.TryProcessHeader() != OperationStatus.Done)
            {
                throw SQL.SynchronousCallMayNotPend();
            }
        ...
        internal bool Run(RunBehavior runBehavior, SqlCommand cmdHandler, SqlDataReader dataStream, BulkCopySimpleResultSet bulkCopyHandler, TdsParserStateObject stateObj)
        {
            ...
            try
            {
                ...
                bool dataReady;
                OperationStatus result = TryRun(runBehavior, cmdHandler, dataStream, bulkCopyHandler, stateObj, out dataReady);
                Debug.Assert(result == OperationStatus.Done, "Should always return Done when _syncOverAsync is set");
                return dataReady;
            }
            finally
            {
        ...
        internal OperationStatus TryRun(RunBehavior runBehavior, SqlCommand cmdHandler, SqlDataReader dataStream, BulkCopySimpleResultSet bulkCopyHandler, TdsParserStateObject stateObj, out bool dataReady)
        {
            ...
            {
                dataReady = true;
                return OperationStatus.Done; // Just in case this is called in a loop, expecting data to be returned.
            }
                    ...
                    case TdsEnums.SQLINFO:
                        {
                            ...
                            SqlError error;
                            result = TryProcessError(token, stateObj, out error);
                            if (result != OperationStatus.Done)
                            {
                                return result;
                            }
        ...
        private OperationStatus TryProcessEnvChange(int tokenLength, TdsParserStateObject stateObj, out SqlEnvChange sqlEnvChange)
        {
            ...
            while (tokenLength > processedLength)
            {
                ...
                result = stateObj.TryReadByte(out env.type);
                if (result != OperationStatus.Done)
                {
                    return result;
                }
                ...
                switch (env.type)
                {
                    ...
                    case TdsEnums.ENV_COLLATION:
                        ...
                        if (env.newLength == 5)
                        {
                            result = TryProcessCollation(stateObj, out env.newCollation);
                            if (result != OperationStatus.Done)
                            {
                                return result;
                            }
                        ...
                        if (env.oldLength == 5)
                        {
                            result = TryProcessCollation(stateObj, out env.oldCollation);
                            if (result != OperationStatus.Done)
                            {
                                return result;
                            }
                    ...
                    case TdsEnums.ENV_TRANSACTIONENDED:
                        ...
                        if (env.newLength > 0)
                        {
                            result = stateObj.TryReadInt64(out env.newLongValue);
                            if (result != OperationStatus.Done)
                            {
                                return result;
                            }
                        ...
                        if (env.oldLength > 0)
                        {
                            result = stateObj.TryReadInt64(out env.oldLongValue);
                            if (result != OperationStatus.Done)
                            {
                                return result;
                            }
                    ...
                    case TdsEnums.ENV_PROMOTETRANSACTION:
                        result = stateObj.TryReadInt32(out env.newLength);
                        if (result != OperationStatus.Done)
                        { // new value has 4 byte length
                            return result;
                        }
                        env.newBinValue = new byte[env.newLength];
                        result = stateObj.TryReadByteArray(env.newBinValue, env.newLength);
                        if (result != OperationStatus.Done)
        ...
        private OperationStatus TryReadTwoBinaryFields(SqlEnvChange env, TdsParserStateObject stateObj)
        {
            // Used by ProcessEnvChangeToken
            byte byteLength;
            OperationStatus result = stateObj.TryReadByte(out byteLength);
            if (result != OperationStatus.Done)
            {
                return result;
            }
            ...
            result = stateObj.TryReadByteArray(env.oldBinValue, env.oldLength);
            if (result != OperationStatus.Done)
            {
                return result;
            }
        ...
        private OperationStatus TryReadTwoStringFields(SqlEnvChange env, TdsParserStateObject stateObj)
        {
            // Used by ProcessEnvChangeToken
            byte newLength, oldLength;
            string newValue, oldValue;
            OperationStatus result = stateObj.TryReadByte(out newLength);
            if (result != OperationStatus.Done)
            {
                return result;
            }
        ...
        private OperationStatus TryProcessDone(SqlCommand cmd, SqlDataReader reader, ref RunBehavior run, TdsParserStateObject stateObj)
        {
            ushort curCmd;
            ushort status;
            int count;
            ...
            OperationStatus result = stateObj.TryReadUInt16(out status);
            if (result != OperationStatus.Done)
            {
                return result;
            }
        ...
        private OperationStatus TryProcessFeatureExtAck(TdsParserStateObject stateObj)
        {
            // read feature ID
            OperationStatus result;
            byte featureId;
            do
            {
                result = stateObj.TryReadByte(out featureId);
                if (result != OperationStatus.Done)
                {
                    return result;
                }
        ...
        private OperationStatus TryReadByteString(TdsParserStateObject stateObj, out string value)
        {
            value = string.Empty;

            byte byteLen;
            OperationStatus result = stateObj.TryReadByte(out byteLen);
            if (result != OperationStatus.Done)
            {
                return result;
            }
        ...
        private OperationStatus TryReadSensitivityLabel(TdsParserStateObject stateObj, out string label, out string id)
        {
            label = string.Empty;
            id = string.Empty;
            OperationStatus result = TryReadByteString(stateObj, out label);
            if (result != OperationStatus.Done)
            {
                return result;
            }
        ...
        private OperationStatus TryReadSensitivityInformationType(TdsParserStateObject stateObj, out string informationType, out string id)
        {
            informationType = string.Empty;
            id = string.Empty;
            OperationStatus result = TryReadByteString(stateObj, out informationType);
            if (result != OperationStatus.Done)
            {
                return result;
            }
        ...
        private OperationStatus TryProcessDataClassification(TdsParserStateObject stateObj, out SensitivityClassification sensitivityClassification)
        {
            ...
            // get the labels
            OperationStatus result = stateObj.TryReadUInt16(out ushort numLabels);
            if (result != OperationStatus.Done)
            {
                return result;
            }
        ...
        private OperationStatus TryProcessResColSrcs(TdsParserStateObject stateObj, int tokenLength)
        {
            return stateObj.TrySkipBytes(tokenLength);
        }

        private OperationStatus TryProcessSessionState(TdsParserStateObject stateObj, int length, SessionData sdata)
        {
            if (length < 5)
            {
                throw SQL.ParsingError();
            ...
            uint seqNum;
            OperationStatus result = stateObj.TryReadUInt32(out seqNum);
            if (result != OperationStatus.Done)
            {
                return result;
            }
        ...
        private OperationStatus TryProcessLoginAck(TdsParserStateObject stateObj, out SqlLoginAck sqlLoginAck)
        {
            SqlLoginAck a = new SqlLoginAck();

            sqlLoginAck = null;
            OperationStatus result = stateObj.TrySkipBytes(1);
        ...
        private OperationStatus TryProcessFedAuthInfo(TdsParserStateObject stateObj, int tokenLen, out SqlFedAuthInfo sqlFedAuthInfo)
        {
            sqlFedAuthInfo = null;
            ...
            uint optionsCount;
            OperationStatus result = stateObj.TryReadUInt32(out optionsCount);
            if (result != OperationStatus.Done)
            {
            ...
            if (tokenLen > 0)
            {
                ...
                int totalRead = 0;
                OperationStatus successfulRead = stateObj.TryReadByteArray(tokenData, tokenLen, out totalRead);
                if (SqlClientEventSource.Log.IsAdvancedTraceOn())
                {
                ...
                if (successfulRead != OperationStatus.Done || totalRead != tokenLen)
                {
        ...
        internal OperationStatus TryProcessError(byte token, TdsParserStateObject stateObj, out SqlError error)
        {
            ushort shortLen;
            byte byteLen;
            int number;
            ...
            error = null;
            OperationStatus result = stateObj.TryReadInt32(out number);
            if (result != OperationStatus.Done)
            {
                return result;
            }
        ...
        internal OperationStatus TryProcessReturnValue(int length, TdsParserStateObject stateObj, out SqlReturnValue returnValue, SqlCommandColumnEncryptionSetting columnEncryptionSetting)
        {
            returnValue = null;
            SqlReturnValue rec = new SqlReturnValue();
            rec.length = length;        // In Yukon this length is -1
            ushort parameterIndex;
            OperationStatus result = stateObj.TryReadUInt16(out parameterIndex);
            if (result != OperationStatus.Done)
            {
                return result;
            }
        ...
        internal OperationStatus TryProcessTceCryptoMetadata(TdsParserStateObject stateObj,
            SqlMetaDataPriv col,
        ...
        internal OperationStatus TryProcessCollation(TdsParserStateObject stateObj, out SqlCollation collation)
        {
            SqlCollation newCollation = new SqlCollation();
            OperationStatus result = stateObj.TryReadUInt32(out newCollation.info);
            if (result != OperationStatus.Done)
            {
                collation = null;
                return result;
            }
            result = stateObj.TryReadByte(out newCollation.sortId);
            if (result != OperationStatus.Done)
            {
                collation = null;
                return result;
        ...
        internal void DrainData(TdsParserStateObject stateObj)
        {
            try
            {
                ...
                if (sharedState != null && sharedState._dataReady)
                {
                    ...
                    {
                        if (stateObj.TryProcessHeader() != OperationStatus.Done)
                        {
                            throw SQL.SynchronousCallMayNotPend();
                        }
                    }
                    ...
                    {
                        OperationStatus result = stateObj.Parser.TrySkipRow(stateObj._cleanupMetaData, stateObj);
                    ...
                    else
                    {
                        ...
                        if (sharedState._nextColumnDataToRead < sharedState._nextColumnHeaderToRead)
                        {
                            if ((sharedState._nextColumnHeaderToRead > 0) && (metadata[sharedState._nextColumnHeaderToRead - 1].metaType.IsPlp))
                            {
                                ...
                                {
                                    ulong ignored;
                                    if (TrySkipPlpValue(ulong.MaxValue, stateObj, out ignored) != OperationStatus.Done)
                                    {
                                        throw SQL.SynchronousCallMayNotPend();
                                    }
                                }
                            ...
                            {
                                if (stateObj.TrySkipLongBytes(sharedState._columnDataBytesRemaining) != OperationStatus.Done)
                                {
                                    throw SQL.SynchronousCallMayNotPend();
                                }
                            }
                        ...
                        if (stateObj.Parser.TrySkipRow(metadata, sharedState._nextColumnHeaderToRead, stateObj) != OperationStatus.Done)
                        {
                            throw SQL.SynchronousCallMayNotPend();
                        }
                    }
        ...
        internal OperationStatus TryProcessAltMetaData(int cColumns, TdsParserStateObject stateObj, out _SqlMetaDataSet metaData)
        {
            ...
            result = stateObj.TryReadUInt16(out altMetaDataSet.id);
            if (result != OperationStatus.Done)
            {
                return result;
            }
        ...
        /// </summary>
        internal OperationStatus TryReadCipherInfoEntry(TdsParserStateObject stateObj, out SqlTceCipherInfoEntry entry)
        {
            ...
            // Read the DB ID
            int dbId;
            result = stateObj.TryReadInt32(out dbId);
            if (result != OperationStatus.Done)
            {
                return result;
            }
        ...
        /// </summary>
        internal OperationStatus TryProcessCipherInfoTable(TdsParserStateObject stateObj, out SqlTceCipherInfoTable cipherTable)
        {
            // Read count
            short tableSize = 0;
            cipherTable = null;
            OperationStatus result = stateObj.TryReadInt16(out tableSize);
            if (result != OperationStatus.Done)
            {
                return result;
            }
        ...
        internal OperationStatus TryProcessMetaData(int cColumns, TdsParserStateObject stateObj, out _SqlMetaDataSet metaData, SqlCommandColumnEncryptionSetting columnEncryptionSetting)
        {
        ...
        private OperationStatus TryProcessTypeInfo(TdsParserStateObject stateObj, SqlMetaDataPriv col, UInt32 userType)
        {
            byte byteLen;
            byte tdsType;
            OperationStatus result = stateObj.TryReadByte(out tdsType);
            if (result != OperationStatus.Done)
            {
                return result;
            }
        ...
        private OperationStatus TryCommonProcessMetaData(TdsParserStateObject stateObj, _SqlMetaData col, SqlTceCipherInfoTable cipherTable, bool fColMD, SqlCommandColumnEncryptionSetting columnEncryptionSetting)
        {
            byte byteLen;
            uint userType;
            OperationStatus result;
        ...
        internal OperationStatus TryProcessTableName(int length, TdsParserStateObject stateObj, out MultiPartTableName[] multiPartTableNames)
        {
            int tablesAdded = 0;
            ...
            while (length > 0)
            {
                OperationStatus result = TryProcessOneTable(stateObj, ref length, out mpt);
                if (result != OperationStatus.Done)
                {
        ...
        private OperationStatus TryProcessOneTable(TdsParserStateObject stateObj, ref int length, out MultiPartTableName multiPartTableName)
        {
            ushort tableLen;
            MultiPartTableName mpt;
            string value;
            ...
            result = stateObj.TryReadByte(out nParts);
            if (result != OperationStatus.Done)
            {
                return result;
            }
        ...
        private OperationStatus TryProcessColInfo(_SqlMetaDataSet columns, SqlDataReader reader, TdsParserStateObject stateObj, out _SqlMetaDataSet metaData)
        {
            ...
            for (int i = 0; i < columns.Length; i++)
            {
                ...
                byte ignored;
                OperationStatus result = stateObj.TryReadByte(out ignored);
                if (result != OperationStatus.Done)
                {
                    return result;
                }
        ...
        internal OperationStatus TryProcessColumnHeader(SqlMetaDataPriv col, TdsParserStateObject stateObj, int columnOrdinal, out bool isNull, out ulong length)
        {
        ...
        private OperationStatus TryProcessColumnHeaderNoNBC(SqlMetaDataPriv col, TdsParserStateObject stateObj, out bool isNull, out ulong length)
        {
            OperationStatus result;
            if (col.metaType.IsLong && !col.metaType.IsPlp)
            {
            ...
            else
            {
                // non-blob columns
                ulong longlen;
                result = TryGetDataLength(col, stateObj, out longlen);
                if (result != OperationStatus.Done)
                {
                    isNull = false;
                    length = 0;
        ...
        internal OperationStatus TryGetAltRowId(TdsParserStateObject stateObj, out int id)
        {
            byte token;
            OperationStatus result = stateObj.TryReadByte(out token);
            if (result != OperationStatus.Done)
        ...
        private OperationStatus TryProcessRow(_SqlMetaDataSet columns, object[] buffer, int[] map, TdsParserStateObject stateObj)
        {
            ...
            for (int i = 0; i < columns.Length; i++)
            {
                ...
                bool isNull;
                ulong len;
                OperationStatus result = TryProcessColumnHeader(md, stateObj, i, out isNull, out len);
                if (result != OperationStatus.Done)
                {
                    return result;
                }
        ...
        internal  OperationStatus TrySkipRow(_SqlMetaDataSet columns, TdsParserStateObject stateObj)
        {
            return TrySkipRow(columns, 0, stateObj);
        }

        internal OperationStatus TrySkipRow(_SqlMetaDataSet columns, int startCol, TdsParserStateObject stateObj)
        {
            for (int i = startCol; i < columns.Length; i++)
            {
                _SqlMetaData md = columns[i];
                OperationStatus result = TrySkipValue(md, i, stateObj);
                if (result != OperationStatus.Done)
                {
                    return result;
                }
        ...
        /// </summary>
        internal OperationStatus TrySkipValue(SqlMetaDataPriv md, int columnOrdinal, TdsParserStateObject stateObj)
        {
            if (stateObj.IsNullCompressionBitSet(columnOrdinal))
            {
                return OperationStatus.Done;
            ...
            {
                ulong ignored;
                OperationStatus result = TrySkipPlpValue(ulong.MaxValue, stateObj, out ignored);
                if (result != OperationStatus.Done)
                {
                    return result;
                }
            ...
            else if (md.metaType.IsLong)
            {
                ...
                byte textPtrLen;
                OperationStatus result = stateObj.TryReadByte(out textPtrLen);
                if (result != OperationStatus.Done)
                {
                    return result;
                }
            ...
            {
                int length;
                OperationStatus result = TryGetTokenLength(md.tdsType, stateObj, out length);
                if (result != OperationStatus.Done)
                {
                    return result;
                }
        ...
        private OperationStatus TryReadSqlStringValue(SqlBuffer value, byte type, int length, Encoding encoding, bool isPlp, TdsParserStateObject stateObj)
        {
            OperationStatus result;
            switch (type)
            {
        ...
        internal OperationStatus TryReadSqlValue(SqlBuffer value, SqlMetaDataPriv md, int length, TdsParserStateObject stateObj, SqlCommandColumnEncryptionSetting columnEncryptionOverride, string columnName, SqlCommand command = null)
        {
        ...
        private OperationStatus TryReadSqlDateTime(SqlBuffer value, byte tdsType, int length, byte scale, TdsParserStateObject stateObj)
        {
            Span<byte> datetimeBuffer = ((uint)length <= 16) ? stackalloc byte[16] : new byte[length];
            OperationStatus result = stateObj.TryReadByteArray(datetimeBuffer, length);
            if (result != OperationStatus.Done)
            {
                return result;
            }
        ...
        internal OperationStatus TryReadSqlValueInternal(SqlBuffer value, byte tdsType, int length, TdsParserStateObject stateObj)
        {
            OperationStatus result;
            switch (tdsType)
            {
        ...
        //      BYTE[] DataVal
        // }
        internal OperationStatus TryReadSqlVariant(SqlBuffer value, int lenTotal, TdsParserStateObject stateObj)
        {
            OperationStatus result;
            // get the SQLVariant type
            byte type;
            result = stateObj.TryReadByte(out type);
            if (result != OperationStatus.Done)
            {
                return result;
            }
        ...
        private byte[] SerializeDateTimeOffset(DateTimeOffset value, byte scale, int length)
        {
            ...
            short offset = (Int16)value.Offset.TotalMinutes;
        ...
        private OperationStatus TryReadSqlDecimal(SqlBuffer value, int length, byte precision, byte scale, TdsParserStateObject stateObj)
        {
            byte byteValue;
            OperationStatus result = stateObj.TryReadByte(out byteValue);
            if (result != OperationStatus.Done)
            {
                return result;
            }
        ...
        private OperationStatus TryReadDecimalBits(int length, TdsParserStateObject stateObj, out int[] bits)
        {
            ...
            {
                // up to 16 bytes of data following the sign byte
                OperationStatus result = stateObj.TryReadInt32(out bits[i]);
                if (result != OperationStatus.Done)
                {
                    return result;
                }
        ...
        //
        internal OperationStatus TryGetDataLength(SqlMetaDataPriv colmeta, TdsParserStateObject stateObj, out ulong length)
        {
            ...
            {
                int intLength;
                OperationStatus result = TryGetTokenLength(colmeta.tdsType, stateObj, out intLength);
                if (result != OperationStatus.Done)
                {
                    length = 0;
                    return result;
        ...
        //
        internal OperationStatus TryGetTokenLength(byte token, TdsParserStateObject stateObj, out int tokenLength)
        {
...

            {
                ...
                {
                    ushort value;
                    OperationStatus result = stateObj.TryReadUInt16(out value);
                    if (result != OperationStatus.Done)
                    {
                        tokenLength = 0;
                        return result;
            ...
            switch (token & TdsEnums.SQLLenMask)
            {
                ...
                case TdsEnums.SQLVarCnt:
                    ...
                    {
                        OperationStatus result = stateObj.TryReadInt32(out tokenLength);
                        if (result != OperationStatus.Done)
                        {
                            return result;
                        }
                    ...
                    {
                        byte value;
                        OperationStatus result = stateObj.TryReadByte(out value);
                        if (result != OperationStatus.Done)
                        {
                            tokenLength = 0;
                            return result;
        ...
        private void ProcessSSPI(int receivedLength)
        {
            ...
            Debug.Assert(_physicalStateObj._syncOverAsync, "Should not attempt pends in a synchronous call");
            OperationStatus result = _physicalStateObj.TryReadByteArray(receivedBuff, receivedLength);
            if (result != OperationStatus.Done)
            {
                throw SQL.SynchronousCallMayNotPend();
            }
        ...
        private OperationStatus TryReadPlpUnicodeCharsChunk(char[] buff, int offst, int len, TdsParserStateObject stateObj, out int charsRead)
        {
            ...
            {
                OperationStatus result = stateObj.TryReadChar(out buff[offst + ii]);
                if (result != OperationStatus.Done)
                {
                    return result;
                }
        ...
        internal int ReadPlpUnicodeChars(ref char[] buff, int offst, int len, TdsParserStateObject stateObj)
        {
            int charsRead;
            Debug.Assert(stateObj._syncOverAsync, "Should not attempt pends in a synchronous call");
            OperationStatus result = TryReadPlpUnicodeChars(ref buff, offst, len, stateObj, out charsRead);
            if (result != OperationStatus.Done)
            {
                throw SQL.SynchronousCallMayNotPend();
            }
        ...
        // Returns the actual chars read.
        internal OperationStatus TryReadPlpUnicodeChars(ref char[] buff, int offst, int len, TdsParserStateObject stateObj, out int totalCharsRead)
        {
            int charsRead = 0;
            int charsLeft = 0;
            char[] newbuf;
            ...
            while (charsLeft > 0)
            {
                ...
                if (charsRead > 0)
                {
                    OperationStatus result = TryReadPlpUnicodeCharsChunk(buff, offst, charsRead, stateObj, out charsRead);
                    if (result != OperationStatus.Done)
                    {
                        return result;
                    }
        ...
        internal ulong SkipPlpValue(ulong cb, TdsParserStateObject stateObj)
        {
            ulong skipped;
            Debug.Assert(stateObj._syncOverAsync, "Should not attempt pends in a synchronous call");
            OperationStatus result = TrySkipPlpValue(cb, stateObj, out skipped);
            if (result != OperationStatus.Done)
            {
                throw SQL.SynchronousCallMayNotPend();
            }
        ...
        internal OperationStatus TrySkipPlpValue(ulong cb, TdsParserStateObject stateObj, out ulong totalBytesSkipped)
        {
        ...
        private OperationStatus TryProcessUDTMetaData(SqlMetaDataPriv metaData, TdsParserStateObject stateObj)
        {
            ushort shortLength;
            byte byteLength;
            OperationStatus result = stateObj.TryReadUInt16(out shortLength);
            if (result != OperationStatus.Done)
            {
                return result;
            }
