...
{		        
	...
	if( !(gDsFunc.DsCrackNames 
		= (DSCRACKNAMES_FN) GetProcAddress( ghSpnLib, DsFunctionNames[iDsCrackNames])) )
		goto ErrorExit;
...
DWORD SNI_Spn::AddRemoveSpn(__in LPCTSTR pszInstanceSPN, DWORD dwPortNum, BOOL fAdd, DWORD * pdwState)
{
	...
	*pdwState = x_earsSuccess; 
		...
		&& _tcsicmp_l((_TCHAR *) wszUserName, _T("NETWORK SERVICE"), GetDefaultLocale()))
	{
		...
		dwError = gDsFunc.DsCrackNames( hDs,
		                            DS_NAME_NO_FLAGS,
		                            DS_NT4_ACCOUNT_NAME,
		                            DS_FQDN_1779_NAME,
		                            1,
		...
		if(dwError != NO_ERROR)
		{
			*pdwState = x_earsDsCrackNames;
			BidTrace1( ERROR_TAG 
					_T("DsCrackNames: %d{WINERR}\n"), 
					dwError );
			goto Exit;
		}
		...
		else
		{
			...
			*pdwState = x_earsDsCrackNamesInvalidData;
			BidTrace0( ERROR_TAG 
					_T("Invalid data from DsCrackNames\n") );
			goto Exit;
		}
