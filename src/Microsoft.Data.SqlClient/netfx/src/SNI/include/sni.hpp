...
typedef DWORD (WINAPI *PFnSniClientCertificateFallback)(
...
//----------------------------------------------------------------------------
// Name: 	SNIAuthProviderInfo
//
// Purpose:	Parameter bag to pass to SNIAddProviderEx
//			
//----------------------------------------------------------------------------
class SNIAuthProviderInfo
{
public:
	DWORD			    dwFlags;
	...
	PFnSniClientCertificateFallback pfnCertificateCallback;	
};

//----------------------------------------------------------------------------
// Name:    SNICTAIPProviderInfo
//
// Purpose: Parameter bag to pass to SNIAddProviderEx
//          
//----------------------------------------------------------------------------
class SNICTAIPProviderInfo
{
public:
    SNI_Conn           *pConn;
    LPCBYTE            prgbAddress;
    ULONG              cbAddress;
    BOOL               fFromDataSecurityProxy;
};

//----------------------------------------------------------------------------
// Name:    SNICTAIPAdressInfo
//
// Purpose: Parameter bag to pass to SNIGetInfoEx
//          
//----------------------------------------------------------------------------
class SNICTAIPAddressInfo
{
public:
    BYTE               rgbAddress[16];
    ULONG              cbAddress;
...
//          sni_error.cpp, to the SNI_STRING_PROV_* ID's defined
//          in sni_rc.h, and to the Provider descriptions in 
//          system.data.txt.
//
//----------------------------------------------------------------------------
typedef enum 
...
#endif
    SNI_QUERY_CONN_TOP_PROVIDERNUM,
    SNI_QUERY_CONN_IPADDR,
} QTypes;
...
{ 
	...
	// TODO: These are server-specific parameters. They should be moved out to an SNI_SERVER_CONSUMER_INFO struct at a later date, 
	// which will contain an SNI_CONSUMER_INFO as a member (similar to the SNI_CLIENT_CONSUMER_INFO below)
