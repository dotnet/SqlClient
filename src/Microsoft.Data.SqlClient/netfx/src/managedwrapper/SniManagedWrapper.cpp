...
DWORD WINAPI SNIClientCertificateFallbackWrapper(
...
ref class  SNINativeMethodWrapper
{
     internal :
        ...
        delegate IntPtr SqlClientCertificateDelegate(System::IntPtr pCallbackContext);
        ...
        [class:CLSCompliant(false)]
        ref class AuthProviderInfo
        {
                internal:
                    System::UInt32      flags;
                    System::String^     certId;
                    System::Boolean     certHash;
                    System::Object^ clientCertificateCallbackContext;
                    SqlClientCertificateDelegate^ clientCertificateCallback;
        };

        [class:CLSCompliant(false)]
        ref class CTAIPProviderInfo
        {
                internal:
                    cli::array<System::Byte>^ originalNetworkAddress;
                    System::Boolean     fromDataSecurityProxy;
        };
...
internal:
                                  ...
                                  System::IntPtr % ret)
    {
        ...
        {
            DWORD _info =  info;
            System::UInt32 ret = SNINativeMethodWrapper::SNIAddProviderIntenal(pConn, providerEnum, &_info);
            info = _info;
            return ret;
        }
        ...
        {
            System::UInt32 ret;
            cli::pin_ptr<const WCHAR>  pin_certId = PtrToStringChars (authInfo->certId);

            SNIAuthProviderInfo sniAuthInfo;
            sniAuthInfo.dwFlags = authInfo->flags;
            sniAuthInfo.pCertContext = NULL;
            sniAuthInfo.pwszCertId = pin_certId;
            sniAuthInfo.fCertHash = authInfo->certHash;
            sniAuthInfo.pCallbackContext = NULL;
            sniAuthInfo.pfnCertificateCallback = NULL;

            SNIAuthProviderInfoWrapper sniAuthInfoWrapper;

            if (authInfo->clientCertificateCallback != nullptr)
            {
                sniAuthInfoWrapper.pDelegateContext = GCHandle::ToIntPtr(GCHandle::Alloc(authInfo->clientCertificateCallbackContext)).ToPointer();
                sniAuthInfoWrapper.pSqlClientCertificateDelegate = (tSqlClientCertificateDelegate)Marshal::GetFunctionPointerForDelegate (authInfo->clientCertificateCallback).ToPointer();

                sniAuthInfo.pCallbackContext = &sniAuthInfoWrapper;
                sniAuthInfo.pfnCertificateCallback = SNIClientCertificateFallbackWrapper;
            }
            
            ret = SNINativeMethodWrapper::SNIAddProviderIntenal(pConn, providerEnum, &sniAuthInfo);

            if (authInfo->clientCertificateCallback != nullptr)
            {
                GCHandle::FromIntPtr(IntPtr(sniAuthInfoWrapper.pDelegateContext)).Free();
            }
        ...
        {
            System::UInt32 ret;
            cli::pin_ptr<const BYTE>  pin_address = &authInfo->originalNetworkAddress[0];

            SNICTAIPProviderInfo ctaipInfo = {0};
            ctaipInfo.prgbAddress = pin_address;
            ctaipInfo.cbAddress = authInfo->originalNetworkAddress->Length;
            ctaipInfo.fFromDataSecurityProxy = authInfo->fromDataSecurityProxy;
            
            ret = SNINativeMethodWrapper::SNIAddProviderIntenal(pConn, providerEnum, &ctaipInfo);

            return ret;
        }

        static System::UInt32 SNIAddProviderIntenal(SafeHandle^  pConn,
                                              ProviderEnum providerEnum,
                                              void *info)
        {
            System::UInt32 ret;
            ...
            __try
            {
                ...
                SNI_ConnWrapper*  local_pConn = static_cast<SNI_ConnWrapper*>(pConn->DangerousGetHandle().ToPointer ());
                ret = ::SNIAddProvider ( local_pConn->m_pConn, static_cast<ProviderNum>(providerEnum), info);
        ...
        [ResourceExposure(ResourceScope::None)]
        [ResourceConsumption(ResourceScope::Machine, ResourceScope::Machine)]
        static System::UInt32 SNISecGetServerCertificate (SafeHandle^ pConnectionObject,
                                                           X509Certificate2^% certificate)
        {
            System::UInt32 ret;
            PCtxtHandle pSecHandle = NULL;
            ...
            __try
            {
                ...
                SNI_ConnWrapper*  local_pConn = static_cast<SNI_ConnWrapper*>(pConnectionObject->DangerousGetHandle().ToPointer ());

                ret = SNIGetInfo(local_pConn->m_pConn, SNI_QUERY_CONN_SSL_SECCTXTHANDLE, (void*)&pSecHandle);
                if (ret == ERROR_SUCCESS)
                {
                    if (SEC_E_OK == (ret = QueryContextAttributes(pSecHandle, SECPKG_ATTR_REMOTE_CERT_CONTEXT, &pCertContext)))
                    {
                        certificate = gcnew X509Certificate2((IntPtr)pCertContext);
                    }
                }
            }
            __finally
