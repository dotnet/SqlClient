...
namespace Microsoft.Data.SqlClient
{
    internal static class SNINativeMethodWrapper
    {
        ...
        [DllImport(SNI, CallingConvention = CallingConvention.Cdecl)]
        internal static extern int SNISecADALInitialize();

        [DllImport(SNI, CallingConvention = CallingConvention.Cdecl)]
        internal static extern uint SNISecADALGetAccessToken(String username,
                                            String password,
                                            String stsURL,
                                            String servicePrincipalName,
                                            Guid correlationId,
                                            String clientId,
                                            bool fWindowsIntegrated,
                                            String pToken,
                                            uint cbToken,
                                            String pErrorDescription,
                                            uint errorDescriptionLength,
                                            uint adalStatus, 
                                            uint errorState,       
                                            System.Runtime.InteropServices.ComTypes.FILETIME fileTimeLocal);
...
class ADALNativeWrapper
{
    ...
    {
         return SNINativeMethodWrapper.SNISecADALInitialize();
    }

    internal static byte[] ADALGetAccessToken(String username,
                                           ...
                                           Int64 fileTime)
    {
        Debug.Assert(password != null, "Password from SecureString is null.");
        ...
        try
        {
            ...
            byte[] result = ADALGetAccessToken(username,
                                                      clearPassword,
                                                      stsURL,
    ...
    internal static byte[] ADALGetAccessTokenForWindowsIntegrated(String stsURL,
                                                                String servicePrincipalName,
                                                                Guid correlationId,
                                                                String clientId,
                                                                ref Int64 fileTime)
    {
        return ADALGetAccessToken(null /*username*/,
    ...
    private static byte[] ADALGetAccessToken(String username,
                                            IntPtr password,
                                            String stsURL,
                                            String servicePrincipalName,
                                            Guid correlationId,
                                            ...
                                            Int64 fileTime)
    {
        Debug.Assert(username != null || fWindowsIntegrated, "User name is null and its not windows integrated authentication.");
        ...
        {
            pUsername = username;
            pPassword = Marshal.PtrToStringAuto(password);
        }

        string pStsURL = stsURL;
        string pServicePrincipalName = servicePrincipalName;
        string pClientId = clientId;

        string pToken = null;
        string pErrorDescription = null;
        uint cbToken = 0, errorDescriptionLength = 0;
        uint adalStatus = 0;//ERROR_SUCCESS;
        uint errorState = 0;
        System.Runtime.InteropServices.ComTypes.FILETIME fileTimeLocal = new System.Runtime.InteropServices.ComTypes.FILETIME { };
        Guid correlationIdGUID = correlationId;
        uint statusCategory = SNINativeMethodWrapper.SNISecADALGetAccessToken(pUsername,
                                                        pPassword,
                                                        pStsURL,
        ...
        if (statusCategory == 0)
        {
            Debug.Assert(pToken != null, "pToken is null.");
            Debug.Assert(cbToken > 0, "token length is less than or equal to 0.");
            Debug.Assert(pErrorDescription == null, "pErrorDescription is not null");
            Debug.Assert(errorDescriptionLength == 0, "ErrorDescription length is not 0.");

            IntPtr ptrToken = Marshal.StringToHGlobalUni(pToken);

            byte[] result = BitConverter.GetBytes(cbToken);
            Marshal.Copy(ptrToken, result, 0, (int)cbToken);

            fileTime = (((int)fileTimeLocal.dwHighDateTime) << 32) + fileTimeLocal.dwLowDateTime;

            return result;
        }
        else
        {
            Debug.Assert(pToken == null, "pToken is not null in error case.");
            Debug.Assert(cbToken == 0, "Token length is not 0 in error case.");

            String errorString = String.Empty;
            if (pErrorDescription != null && errorDescriptionLength >= 0)
            {
                errorString = Marshal.PtrToStringUni(Marshal.StringToHGlobalUni(pErrorDescription), (int)errorDescriptionLength);

            }

            throw new AdalException(errorString, statusCategory, adalStatus, errorState);
        }
    }
}
