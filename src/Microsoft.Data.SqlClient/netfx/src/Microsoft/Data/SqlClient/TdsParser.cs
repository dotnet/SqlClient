...
namespace Microsoft.Data.SqlClient
{
    ...
    sealed internal class TdsParser
    {
        ...
        // Version variables
        private bool _isSql2000; // set to true if we connect to a 8.0 server (SQL 2000) or later

        private bool _isSql2000SP1; // set to true if speaking to 2000 SP1 or later

        private bool _isSql2005; // set to true if speaking to 2005 or later

        private bool _isSql2008;

        private bool _isSql2012;
        ...
        internal bool IsSql2005OrNewer
        {
            get
            {
                return _isSql2005;
        ...
        private bool IncludeTraceHeader
        {
            get
            {
                return (_isSql2012 && SqlClientEventSource.Log.IsEnabled());
            }
        }
            ...
            if (_fResetConnection && !stateObj._fResetConnectionSent)
            {
                ...
                try
                {
                    ...
                    if (_fResetConnection)
                    {
                        ...
                        Debug.Assert(_isSql2000, "TdsParser.cs: Error!  _fResetConnection true when not going against 2000 or later!");
            ...
            while (tokenLength > processedLength)
            {
                ...
                switch (env.type)
                {
                    ...
                    case TdsEnums.ENV_CHARSET:
                        ...
                        Debug.Assert(!_isSql2000, "Received ENV_CHARSET on non 7.0 server!");
                        if (!TryReadTwoStringFields(env, stateObj))
                        {
                            return false;
                        }
                    ...
                    case TdsEnums.ENV_DEFECTDTC:
                    case TdsEnums.ENV_TRANSACTIONENDED:
                        Debug.Assert(_isSql2005, "Received new ENVCHANGE transaction/DTC token on pre 9.0 server!");
                    ...
                    case TdsEnums.ENV_PROMOTETRANSACTION:
                        Debug.Assert(_isSql2005, "Received new ENVCHANGE tokens on pre 9.0 server!");
                    ...
                    case TdsEnums.ENV_USERINSTANCE:
                        Debug.Assert(!_isSql2005, "Received ENV_USERINSTANCE on non 9.0 server!");
                        if (!TryReadTwoStringFields(env, stateObj))
                        {
                            return false;
                        }
        ...
        private bool TryProcessLoginAck(TdsParserStateObject stateObj, out SqlLoginAck sqlLoginAck)
        {
            ...
            switch (majorMinor)
            {
                case TdsEnums.SQL70OR2000_MAJOR << 24 | TdsEnums.DEFAULT_MINOR:    // 7.0 & 2000 RTM
                    // note that 7.0 and 2000 RTM can only be distinguished by the increment
                    switch (increment)
                    {
            ...
            _isSql2008 |= _isSql2012;
            _isSql2005 |= _isSql2008;
            _isSql2000SP1 |= _isSql2005;            // includes all lower versions
            _isSql2000 |= _isSql2000SP1;        //

            a.isVersion8 = _isSql2000;
                                            ...
                                            SqlCommandColumnEncryptionSetting columnEncryptionSetting)
        {
            ...
            if (_isSql2000)
            {
                ...
                {
                    Debug.Assert(_isSql2005, "plp data from pre-2005 server");
            ...
            else if (_isSql2000 && rec.metaType.IsCharType)
            {
        ...
        private bool TryProcessTypeInfo(TdsParserStateObject stateObj, SqlMetaDataPriv col, UInt32 userType)
        {
            ...
            if (_isSql2000 && col.metaType.IsCharType && (tdsType != TdsEnums.SQLXMLTYPE))
            {
        ...
        {
            Debug.Assert(_isSql2000 == true, "Shouldn't be dealing with sql_variaint in pre-SQL2000 server!");
        ...
        {
            Debug.Assert(_isSql2000 == true, "Shouldn't be dealing with sql_variant in pre-SQL2000 server!");
        ...
        internal bool TryGetDataLength(SqlMetaDataPriv colmeta, TdsParserStateObject stateObj, out ulong length)
        {
            ...
            if (_isSql2005 && colmeta.metaType.IsPlp)
            {
            ...
            try
            {
                ...
                switch (request)
                {
                    ...
                    case TdsEnums.TransactionManagerRequestType.Begin:
                        Debug.Assert(IsSql2005OrNewer, "Should not be calling TdsExecuteTransactionManagerRequest on pre-2005 clients for BeginTransaction!");
                    ...
                    case TdsEnums.TransactionManagerRequestType.Promote:
                        Debug.Assert(IsSql2005OrNewer, "Should not be calling TdsExecuteTransactionManagerRequest on pre-2005 clients for PromoteTransaction!");
                    ...
                    case TdsEnums.TransactionManagerRequestType.Commit:
                        Debug.Assert(IsSql2005OrNewer, "Should not be calling TdsExecuteTransactionManagerRequest on pre-2005 clients for CommitTransaction!");
                    ...
                    case TdsEnums.TransactionManagerRequestType.Rollback:
                        Debug.Assert(IsSql2005OrNewer, "Should not be calling TdsExecuteTransactionManagerRequest on pre-2005 clients for RollbackTransaction!");
                    ...
                    case TdsEnums.TransactionManagerRequestType.Save:
                        Debug.Assert(IsSql2005OrNewer, "Should not be calling TdsExecuteTransactionManagerRequest on pre-2005 clients for SaveTransaction!");
                ...
                try
                {
                    ...
                    for (int ii = startRpc; ii < rpcArray.Length; ii++)
                    {
                        ...
                        for (int i = (ii == startRpc) ? startParam : 0; i < parameters.Length; i++)
                        {
                            ...
                            if ((!_isSql2000 && !mt.Is70Supported) ||
                                (!_isSql2005 && !mt.Is80Supported) ||
                                (!_isSql2008 && !mt.Is90Supported))
                            {
                            ...
                            else
                            {
                                ...
                                else if (mt.SqlDbType == SqlDbType.Udt)
                                {
                                    ...
                                    Debug.Assert(_isSql2005, "Invalid DataType UDT for non-2005 or later server!");

                                    int maxSupportedSize = IsSql2008OrNewer ? int.MaxValue : short.MaxValue;
                            ...
                            if (_isSql2005 && (mt.SqlDbType == SqlDbType.Xml))
                            {
                            ...
                            else if (_isSql2000 && mt.IsCharType)
                            {
        ...
        internal void WriteTceUserTypeAndTypeInfo(SqlMetaDataPriv mdPriv, TdsParserStateObject stateObj)
        {
            ...
            switch (mdPriv.type)
            {
                ...
                default:
                    WriteTokenLength(mdPriv.tdsType, mdPriv.length, stateObj);
                    if (mdPriv.metaType.IsCharType && _isSql2000)
                    {
                ...
                if (metadataCollection[i] != null)
                {
                    ...
                    switch (md.type)
                    {
                        ...
                        default:
                            stateObj.WriteByte(md.tdsType);
                            WriteTokenLength(md.tdsType, md.length, stateObj);
                            if (md.metaType.IsCharType && _isSql2000)
                            {
        ...
        private void WriteMarsHeaderData(TdsParserStateObject stateObj, SqlInternalTransaction transaction)
        {
            ...
            Debug.Assert(_isSql2005, "WriteMarsHeaderData called on a non-2005 server");
        ...
        {
            Debug.Assert(_isSql2005, "WriteQueryNotificationHeaderData called on a non-2005 server");
        ...
        {
            Debug.Assert(_isSql2005, "WriteRPCBatchHeaders can only be called on 2005 or higher version servers");
        ...
        internal string TraceString()
        {
            return string.Format(/*IFormatProvider*/ null,
                            ...
                            _isSql2000 ? bool.TrueString : bool.FalseString,
                            _isSql2000SP1 ? bool.TrueString : bool.FalseString,
                            _isSql2005 ? bool.TrueString : bool.FalseString,
