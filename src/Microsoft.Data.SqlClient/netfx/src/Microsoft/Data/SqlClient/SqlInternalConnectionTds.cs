...
namespace Microsoft.Data.SqlClient
{
    ...
    sealed internal class SqlInternalConnectionTds : SqlInternalConnection, IDisposable
    {
        ...
        internal SqlInternalConnectionTds(
                ...
                KerberosTicketRetrievalCallback kerberosTicketRetrievalCallback = null,
                ...
                bool applyTransientFaultHandling = false) : base(connectionOptions)
        {
            ...
            if (kerberosTicketRetrievalCallback != null)
            {
                var dataSource = DataSource.ParseServerName(connectionOptions.DataSource);
                Debug.Assert(dataSource.IsBadDataSource, "Provided DataSource is not valid");
                _kerberosTicketInBytes = kerberosTicketRetrievalCallback(dataSource.SPN);
            }
        ...
        private void Login(ServerInfo server, TimeoutTimer timeout, string newPassword, SecureString newSecurePassword)
        {
            ...
            _parser.TdsLogin(login, requestedFeatures, _recoverySessionData, _fedAuthFeatureExtensionData, _kerberosTicketInBytes, _originalNetworkAddressInfo);
    ...
    internal sealed class DataSource
    {
        ...
        private DataSource(string dataSource)
        {
            ...
            Debug.Assert(_dataSourceAfterTrimmingProtocol.Contains(Slash), "pipe paths only allow back slashes");
        }
        ...
        public string SPN
        {
            get
            {
                Debug.Assert(this.IsBadDataSource);

                string hostNameOrAddress = ServerName;
                string portOrInstanceName = null;
                if (Port != -1)
                {
                    portOrInstanceName = Port.ToString();
                }
                else if (!string.IsNullOrWhiteSpace(InstanceName))
                {
                    portOrInstanceName = InstanceName;
                }

                SqlClientEventSource.Log.TryTraceEvent("DataSource.SPN | Info | ServerName {0}, InstanceName {1}, Port {2}, postfix {3}", ServerName, InstanceName, Port, portOrInstanceName);
                ...
                else if (_connectionProtocol == DataSource.Protocol.None || _connectionProtocol == DataSource.Protocol.TCP) // Default is TCP
                {
