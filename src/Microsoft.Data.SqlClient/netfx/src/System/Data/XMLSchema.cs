...
// TODO: May be later we can send better exception message: We have column info, table info, and simple type properties
        ...
        public void LoadSchema(XmlSchemaSet schemaSet , DataSet ds) { //Element schemaRoot, DataSet ds) {
            ...
            if (schemaSet == null) // TODO : we should return for empty set schemaSet.Count == 0
                return;
...
// TODO: Do not do following fix, NS for nested child table is patched later
//            if (table == null && node.RefName.IsEmpty && !IsTopLevelElement(node) && _TableUri != null && _TableUri.Length > 0) { // _TableUri should not be null, but dont rely on Xml
//                _TableUri = null;    // it means form="qualified", so child element inherits namespace.  amihmy
//            }            
/*            if (table!=null) {
...
//TODO:    if (attr.SchemaTypeName.Name != null && attr.SchemaTypeName.Name.Length != 0 && attr.SchemaTypeName.Namespace != Keywords.XSDNS) { // this means UDSimpleType                    
