...
// WebData 88234
                ...
                if (MissingSchemaAction.Ignore != missingSchemaAction) {
                    ...
                    for (int i = 0; i < table.Columns.Count; i++) {
                        ...
                        if (dest == null) {
                            if (MissingSchemaAction.Add == missingSchemaAction) {
                                ...
                                if (!isStandAlonetable)
                                    dataSet.RaiseMergeFailed(targetTable, ResHelper.GetString(Res.DataMerge_MissingColumnDefinition, table.TableName, src.ColumnName), missingSchemaAction);
                                else
                                    throw ExceptionBuilder.MergeFailed(ResHelper.GetString(Res.DataMerge_MissingColumnDefinition, table.TableName, src.ColumnName));
                            }
                        }
                        else {
                            if (dest.DataType != src.DataType || 
                                ...
                                if (!isStandAlonetable)
                                    dataSet.RaiseMergeFailed(targetTable, ResHelper.GetString(Res.DataMerge_DataTypeMismatch, src.ColumnName), MissingSchemaAction.Error);
                                else
                                    throw ExceptionBuilder.MergeFailed(ResHelper.GetString(Res.DataMerge_DataTypeMismatch, src.ColumnName));
                            }
                    ...
                    if (targetPKey.Length != tablePKey.Length) {
                        ...
                        if (targetPKey.Length == 0) {
                            ...
                            dataSet.RaiseMergeFailed(targetTable, ResHelper.GetString(Res.DataMerge_PrimaryKeyMismatch), missingSchemaAction);
                        }
                    }
                    else {
                        for (int i = 0; i < targetPKey.Length; i++) {
                            if (String.Compare(targetPKey[i].ColumnName, tablePKey[i].ColumnName, false, targetTable.Locale) != 0) {
                                dataSet.RaiseMergeFailed(table,
                                    ResHelper.GetString(Res.DataMerge_PrimaryKeyColumnsMismatch, targetPKey[i].ColumnName, tablePKey[i].ColumnName),
                                    missingSchemaAction
                            );
                            }
                        }
        ...
        private void MergeConstraints(DataTable table) {
            ...
            for (int i = 0; i < table.Constraints.Count; i++) {
                ...
                if (dest == null) {
                    dataSet.RaiseMergeFailed(table,
                        ResHelper.GetString(Res.DataMerge_MissingConstraint, src.GetType().FullName, src.ConstraintName),
                        missingSchemaAction
                    );
                }
                else {
            ...
            if (iDest >= 0) {
                ...
                if (relation.ParentKey.ColumnsReference.Length != destRelation.ParentKey.ColumnsReference.Length) {
                    dataSet.RaiseMergeFailed(null,
                        ResHelper.GetString(Res.DataMerge_MissingDefinition, relation.RelationName),
                        missingSchemaAction
                    );
                }
                for (int i = 0; i < relation.ParentKey.ColumnsReference.Length; i++) {
                    ...
                    if (0 != string.Compare(dest.ColumnName, src.ColumnName, false, dest.Table.Locale)) {
                        dataSet.RaiseMergeFailed(null,
                            ResHelper.GetString(Res.DataMerge_ReltionKeyColumnsMismatch, relation.RelationName),
                            missingSchemaAction
                        );
                    }
