...
public sealed class ActiveDirectoryAuthenticationProvider : SqlAuthenticationProviderBase
{
    ...
    private static readonly string _type = typeof(ActiveDirectoryAuthenticationProvider).Name;
    ...
    public override async Task<SqlAuthenticationTokenBase> AcquireTokenAsync(SqlAuthenticationParametersBase parameters)
    {
        ...
        //
        // TODO(ADO-37652): We need a way to remember this between calls to
        // AcquireTokenAsync with the same parameters.
        //
        // TODO(ADO-37652): Is this even something we should be responsible for
        // deciding?
        int unknownErrorRetryPeriod = 100;
        ...
        try
        {
            ...
            **/
            ...
            if (parameters.AuthenticationMethod == SqlAuthenticationMethod.ActiveDirectoryDefault)
            {
                ...
                return new SqlAuthenticationTokenBase(accessToken.Token, accessToken.ExpiresOn);
            }
            ...
            {
                // TODO(ADO-37652): What should we do if the password is null?
                if (parameters.Password is null)
                {
                    throw new NotImplementedException("ActiveDirectoryServicePrincipal authentication method requires a password.");
                }
            ...
            {
                AuthenticationResult? result = await TryAcquireTokenSilent(app, parameters, scopes, cts).ConfigureAwait(false);

                if (result is not null)
                {
                    SqlClientEventSource.Log.TryTraceEvent("AcquireTokenAsync | Acquired access token for Active Directory Integrated auth mode silently. Expiry Time: {0}", result.ExpiresOn);

                    return new SqlAuthenticationTokenBase(result.AccessToken, result.ExpiresOn);
                }

                if (!string.IsNullOrEmpty(parameters.UserId))
                {
                    AuthenticationResult userIdResult = await app.AcquireTokenByIntegratedWindowsAuth(scopes)
                        .WithCorrelationId(parameters.ConnectionId)
                        .WithUsername(parameters.UserId)
                        .ExecuteAsync(cancellationToken: cts.Token)
                        .ConfigureAwait(false);

                    SqlClientEventSource.Log.TryTraceEvent("AcquireTokenAsync | Acquired access token for Active Directory Integrated auth mode with UserId. Expiry Time: {0}", userIdResult.ExpiresOn);

                    return new SqlAuthenticationTokenBase(userIdResult.AccessToken, userIdResult.ExpiresOn);
                }

                AuthenticationResult tokenResult = await app.AcquireTokenByIntegratedWindowsAuth(scopes)
                    .WithCorrelationId(parameters.ConnectionId)
                    .ExecuteAsync(cancellationToken: cts.Token)
                    .ConfigureAwait(false);

                SqlClientEventSource.Log.TryTraceEvent("AcquireTokenAsync | Acquired access token for Active Directory Integrated auth mode. Expiry Time: {0}", tokenResult.ExpiresOn);

                return new SqlAuthenticationTokenBase(tokenResult.AccessToken, tokenResult.ExpiresOn);
            }
            ...
            if (parameters.AuthenticationMethod == SqlAuthenticationMethod.ActiveDirectoryPassword)
            {
                ...
                if (parameters.Password is null)
                {
                    throw new NotImplementedException("ActiveDirectoryPassword authentication method requires a password.");
                }
                ...
                {
                    var silentResult = await TryAcquireTokenSilent(app, parameters, scopes, cts).ConfigureAwait(false);

                    if (silentResult is not null)
                    {
                        SqlClientEventSource.Log.TryTraceEvent("AcquireTokenAsync | Acquired access token for Active Directory Password auth mode silently. Expiry Time: {0}", silentResult.ExpiresOn);

                        return new SqlAuthenticationTokenBase(silentResult.AccessToken, silentResult.ExpiresOn);
                    }
                }
                ...
                parameters.AuthenticationMethod == SqlAuthenticationMethod.ActiveDirectoryDeviceCodeFlow)
            {
                try
                {
                    AuthenticationResult? silentResult = await TryAcquireTokenSilent(app, parameters, scopes, cts).ConfigureAwait(false);

                    if (silentResult is not null)
                    {
                        SqlClientEventSource.Log.TryTraceEvent("AcquireTokenAsync | Acquired access token (silent) for {0} auth mode. Expiry Time: {1}", parameters.AuthenticationMethod, silentResult.ExpiresOn);
                        return new SqlAuthenticationTokenBase(silentResult.AccessToken, silentResult.ExpiresOn);
                    }
                }
                catch (MsalUiRequiredException)
                {
                    ...
                    AuthenticationResult uiResult = await AcquireTokenInteractiveDeviceFlowAsync(app, scopes, parameters.ConnectionId, parameters.UserId, parameters.AuthenticationMethod, cts, _customWebUI, _deviceCodeFlowCallback).ConfigureAwait(false);
                    SqlClientEventSource.Log.TryTraceEvent("AcquireTokenAsync | Acquired access token (interactive) for {0} auth mode. Expiry Time: {1}", parameters.AuthenticationMethod, uiResult.ExpiresOn);
                    return new SqlAuthenticationTokenBase(uiResult.AccessToken, uiResult.ExpiresOn);
                }
        ...
        catch (MsalException ex)
        {
            ...
            {
                // Capture the current retry period.
                var period = unknownErrorRetryPeriod;

                // Double it for the next attempt.
                unknownErrorRetryPeriod *= 2;
    ...
    private IPublicClientApplication CreateClientAppInstance(PublicClientAppKey publicClientAppKey)
    {
        PublicClientApplicationBuilder builder = PublicClientApplicationBuilder
            .CreateWithApplicationOptions(new PublicClientApplicationOptions
            {
                ...
                // TODO(ADO-37652): Should we put these shared values into Extensions?
                ClientName =
                    #if NETFRAMEWORK
                    ...
                    #endif
                ClientVersion = new Version(Assembly.GetAssembly(typeof(ActiveDirectoryAuthenticationProvider))!.GetCustomAttribute<AssemblyInformationalVersionAttribute>()!.InformationalVersion).ToString(),
