...
volatile static LONG _bidInitialized  = 0;
    ...
    static LONG _bidWait4LoadingComplete( void )
    {
       ...
       #ifdef _DEBUG_TRACE_ON       // TODO: Keep this message in retail build ??
        if( _bidLoading != 0 )
        {
            //
...
//
//  TODO: We need the compiler/linker to support something similar to __declspec(selectany),
//  but with the ability to combine the bits contributed by the different object modules.
//
//  Until then, to make sure that the Build Configuration bits provide accurate information,
//  all source files for entire component must be compiled with consistent set of the
//  BID-related definitions.
//
